...
export default defineComponent({
  ...
  setup(props, { slots, emit, attrs }) {
    ...
    const innerValue = computed<UploadFile[]>({
      set(val) {
    ...
    const customUpload = async (options: UploadRequestOptions) => {
      try {
        ...
        innerValue.value = props.limit === 1
          ? [res]
    ...
    const onRemove = (uploadFile: UploadFile, uploadFiles: UploadFiles) => {
      innerValue.value = uploadFiles
      emit('remove', uploadFile, uploadFiles)
    }
    return () => (
      isView.value
        ? (innerValue.value || []).map(v => <ElLink type="primary" href={v?.url} target='_blank'>{v?.name}</ElLink>)
        : <ElUpload
            style={props.style}
            fileList={innerValue.value}
